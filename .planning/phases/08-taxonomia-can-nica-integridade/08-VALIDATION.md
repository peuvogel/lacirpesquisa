...
| Student finds "AVC" and reaches the correct canonical ids | TAX-05 | End-user search behavior in the live picker; the unit test covers the matcher, not the rendered result | Open Mapas → disease picker → type "avc" → confirm the 3 categories appear as normal selectable rows with the explanatory strip (D-18), and that the displayed label is always the official one (D-19) |
