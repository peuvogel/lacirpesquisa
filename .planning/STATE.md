...
status: executing
stopped_at: "Investigacao 09-08 completa: pendencia A (codigos 9/77) resolvida via fonte autoritativa DATASUS + medicao empirica ao vivo; pendencia B (7 categorias delta extremo) caracterizada -- mecanismo IDENT='5' identificado, correcao fora de escopo (aggregate.py, 09-07). 09-10 continua bloqueado so pela pendencia B."
last_updated: "2026-08-10T16:48:34.756Z"
...
Plan: 12 of 14 (09-11 COMPLETO — confirmacao SC-7 contra SP/2019, checkpoint clinico do operador respondido, gate permanente do SC-7 congelado. Investigacao nova 2026-08-10 resolveu a pendencia dos codigos 9/77; 1 pendencia de dados (7 categorias delta extremo, mecanismo IDENT='5' identificado, fix em aggregate.py fora de escopo) ainda bloqueia o 09-10 -- ver "Bloqueios abertos". 09-01/09-03/09-04 permanecem pausados em bloqueio/checkpoint)
...
- **SC-7 (09-08/09-11/investigacao nova 2026-08-10): 1 das 2 pendencias RESOLVIDA, 1 continua bloqueando o 09-10.** Estado do gate apos a investigacao nova (AC/2019, `test_reconcile_gate.py`): `exato=34, explicado=61, inexplicado=3` (era 33/60/5), `result.ok=False` DE PROPOSITO. **Pendencia A (colisao residual codigos 9/77) RESOLVIDA**: faixa correta determinada via fonte autoritativa DATASUS (`mxcid10lm.htm`, tabela oficial Lista de Tabulacao para Morbidade) + validacao empirica ao vivo (TabNet + microdado, coincidencia exata 26=26 para o codigo 9) -- achado revela uma cadeia de TRES codigos com valores trocados (9/15/77, nao so 9/77), corrigida em `cid-corrections.json`. Libera o efeito das correcoes ja aprovadas: `tuberculose_miliar` (14) agora bate EXATO, `doencas_infecciosas_e_parasitarias_congenitas` (274) agora e EXPLICADO. `PENDENTE_colisao_codigos_9_e_77` em `cid-divergencias.json` tem `bloqueiaUpload: false`. **Pendencia B (7 categorias de delta extremo) CONTINUA BLOQUEANDO** -- mecanismo IDENTIFICADO nesta investigacao (campo `IDENT` do microdado, AIH tipo 5 = internacao de longa permanencia, nao filtrada por `aggregate.py`; deltas colapsam de +45%/+108%/+665% para a banda ja aceita ao filtrar so `IDENT='1'`), mas a correcao pertence a `aggregate.py` (09-07), fora do escopo de arquivo da investigacao -- `PENDENTE_sete_categorias_delta_extremo_sp` continua com `bloqueiaUpload: true`. Ver `pipeline/sih/reports/reconciliacao-sc7.md` §"Investigacao nova, 2026-08-10" e `.planning/phases/09-pipeline-confi-vel-coleta-completa/09-08-INVESTIGACAO-SUMMARY.md`. Dono do proximo passo da pendencia B: 09-07 (fix em `aggregate.py`).
...
- **[investigacao nova 2026-08-10] CONTINUA BLOQUEANDO O UPLOAD -- 7 categorias com delta extremo, mecanismo AGORA IDENTIFICADO:** `restante_de_outras_tuberculoses` (+3.451%, faixa CID corrigida nesta investigacao -- residuo agora +33%/+18% com IDENT=1), `demencia` (+665%), `tuberculose_pulmonar` (+108%), `doenca_de_parkinson` (+47,5%), `tuberculose_do_sistema_nervoso` (+47,4%), `doenca_de_alzheimer` (+45,7%), `tuberc_intest_peritonio_glangl_mesentericos` (+44,4%) -- deltas medidos em SP/2019. Mecanismo identificado nesta investigacao: campo `IDENT` (tipo de AIH) do microdado nao e filtrado por `aggregate.py`; `IDENT='5'` (AIH de longa permanencia, renovacao mensal de faturamento, nao nova admissao) concentra 11,5% a 86,0% dos registros destas sete categorias em SP/2019 (contra 2,7% de baseline no dataset inteiro e 0% em condicoes agudas como apendicite/colelitiase) e e 0% em TODAS as sete em AC/2019 -- explica com mecanismo, nao so volume, por que o Acre nunca revelou o defeito. Filtrando so `IDENT='1'`, os deltas colapsam para +3,7% a +21,1% -- a mesma ordem de grandeza da banda de competencia de processamento ja aceita. **NAO resolvido**: a correcao pertence a `aggregate.py`, de propriedade do 09-07, fora do escopo de arquivo desta investigacao. Registrado em `scripts/catalog/cid-divergencias.json` (`PENDENTE_sete_categorias_delta_extremo_sp`, `bloqueiaUpload: true`, `mecanismoIdentificado: true`). Dono do proximo passo: 09-07 (filtrar `IDENT='1'` em `aggregate.py` antes do upload).

## Session Continuity

Last session: 2026-08-10T16:48:34.753Z
Stopped at: Investigacao 09-08 completa: pendencia A (codigos 9/77) resolvida via fonte autoritativa DATASUS + medicao empirica ao vivo; pendencia B (7 categorias delta extremo) caracterizada -- mecanismo IDENT='5' identificado, correcao fora de escopo (aggregate.py, 09-07). 09-10 continua bloqueado so pela pendencia B.
Resume file: None
