...
status: executing
stopped_at: "09-04-COLETA-INCREMENTAL (ad-hoc, sem PLAN.md formal, 2026-08-11): bloqueio de disco da Task 3 do 09-04 RESOLVIDO -- collect.py novo processa uma UF de cada vez (baixa, agrega, persiste em cache_path('agregados/{uf}.parquet'), recicla o parquet bruto), pico de disco vira o tamanho de uma UF (~1,9 GB para SP, a maior) em vez de ~10-13 GB. Corrida real disparada em segundo plano (PID 49618), 2/27 UFs completas ao vivo contra o FTP real (DF, RR), retomada comprovada em producao apos reinicio. Guarda de disco corrigida ao vivo apos achado real (DF, capital federal/polo de referencia, mediu ~12x acima da projecao -- raio de confianca limita contaminacao entre UFs de razao muito distante). Ver 09-04-COLETA-INCREMENTAL-SUMMARY.md."
last_updated: "2026-08-11T01:59:45.000Z"
...
- **09-04 Task 3 (disco): RESOLVIDO 2026-08-11 por `collect.py` (09-04-COLETA-INCREMENTAL, ad-hoc, sem PLAN.md formal).** A corrida completa de ~10-13 GB nunca precisou rodar de uma vez: `collect.py` processa uma UF por vez (baixa via `download_all(only=[...])` existente, agrega via `aggregate_years` existente, persiste as linhas agregadas -- pequenas -- em `cache_path("agregados/{uf}.parquet")`, recicla o parquet bruto daquela UF), fazendo o pico de disco cair para o tamanho da maior UF sozinha (SP, ~1,9 GB projetado a partir de SP/2019 real ja em cache) em vez dos ~10-13 GB da corrida inteira. Ordem `UF_ORDER`: menor->maior (nao a ordem D-23, que ordena agravos, ortogonal -- ver docstring do modulo), para provar o mecanismo barato antes de arriscar SP/MG. Estado por UF (`CollectLedger`, `nunca_iniciado`/`baixado_pendente_agregacao`/`agregado_reciclado`/`falhou`) e uma camada nova e ortogonal ao `FileLedger` do 09-04 -- `ledger.py` intocado, `baixado` continua significando "baixado, hash verificado", nunca "presente em disco agora". **Corrida real disparada em segundo plano** (2026-08-11, PID 49618 apos reinicio limpo): DF e RR completas de verdade contra o FTP real do DataSUS (156 arquivos cada, DF=145.095 linhas/163,45 MB brutos reciclados, RR=37.460 linhas/37,05 MB brutos reciclados), retomada comprovada em producao (nao so em teste) apos reinicio do processo. **Achado real em producao**: a guarda de disco (semente + calibracao adaptativa por medicao real) superestimou ~12x para DF (capital federal, polo de referencia com volume bruto desproporcional a razao do `sih_metric_muni` legado) -- corrigido com um raio de confianca (`RAIO_CONFIANCA_RAZAO=5x`) que impede uma UF de razao pequena de contaminar a projecao de uma UF ordens de grandeza maior (commit `7384dc0`). **Handoff aberto para 09-09/09-10/09-13**: `partitions.py`/`aggregate.py`/`reconcile.py` leem `cache_path("parquet")` (o BRUTO) diretamente -- depois que `collect.py` reciclar uma UF, essas CLIs vao encontrar "nada a processar" para ela; precisam ser adaptadas para ler de `cache_path("agregados/*.parquet")` no lugar (fora do escopo desta plan ad-hoc, que nao tocou `partitions.py`/`aggregate.py`/`reconcile.py` por `file_scope`). Bloqueia DATA-01/DATA-02 (cobertura real dos 331 agravos) so ate a corrida (dias, por design D-01) completar as 27 UFs -- em andamento, nao mais parada. Ver `09-04-COLETA-INCREMENTAL-SUMMARY.md`.
...
Last session: 2026-08-11T01:59:45.000Z
Stopped at: 09-04-COLETA-INCREMENTAL (ad-hoc, sem PLAN.md formal): bloqueio de disco da Task 3 do 09-04 RESOLVIDO por `collect.py` novo (laco incremental por UF, guarda de disco, CollectLedger ortogonal ao FileLedger). Corrida real disparada em segundo plano (PID 49618), DF e RR completas de verdade contra o FTP real, retomada comprovada em producao. Corrida continua rodando (dias, por design D-01) -- nao esperar terminar antes de retomar outro trabalho da fase. Handoff aberto: partitions.py/aggregate.py/reconcile.py precisam ser adaptados para ler cache_path("agregados/*.parquet") no lugar do parquet bruto que collect.py recicla. Ver 09-04-COLETA-INCREMENTAL-SUMMARY.md.
Resume file: None
