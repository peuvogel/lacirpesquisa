/**
 * Typed port of `assets/js/datasus-normalizer.js` (864 lines, ported in
 * full). Logic is byte-for-byte unchanged from the legacy module — column
 * role → canonical-row transform, metric/category/time option builders, and
 * the Phase 2+ statistical derivation functions (`deriveIndependentTTest`,
 * `derivePairedTTest`, `deriveCorrelationPairs`, `derivePraisSeries`) are
 * ported as-is per 01-03-PLAN.md Task 3 step 3: they are reachable from
 * this same file and Phase 2 will need them, but their numerics are NOT
 * exercised by any Phase 1 test — see 01-03-SUMMARY.md.
 */
...
import {
  ...
  parseDatasusNumber,
} from './datasusImporter';
import type {
  DatasusBestPairResult,
  DatasusCategoryOption,
  DatasusColumnRole,
  DatasusCorrelationPair,
  DatasusCorrelationResult,
  DatasusDerivedRow,
  DatasusFormatType,
  DatasusIndependentTTestResult,
  DatasusMapping,
  DatasusMappingColumn,
  DatasusMetricOption,
  DatasusOmittedRow,
  DatasusPairedRow,
  DatasusPairedTTestResult,
  DatasusParsedOk,
  DatasusPraisSeriesResult,
  DatasusPraisSeriesRow,
  DatasusRow,
  DatasusSource,
  DatasusTimeOption,
  FilteredDatasusRecord,
  LegacyStatsAdapter,
  LegacyUtilsAdapter,
  NormalizedDatasusPreviewRow,
  NormalizedDatasusRecord,
  NormalizedDatasusResult,
  NormalizedDatasusSchema,
  NormalizedDatasusSummary,
  TimeKeyInfo,
} from './types';

type MaybeUtils = LegacyUtilsAdapter | undefined;
type MaybeStats = LegacyStatsAdapter | undefined;
type MaybeMapping = DatasusMapping | null | undefined;
type MaybeParsed = DatasusParsedOk | null | undefined;

function buildUniqueKey(label: string, usedKeys: Set<string>): string {
...
function roleColumns(mapping: MaybeMapping, role: DatasusColumnRole): DatasusMappingColumn[] {
  return (mapping?.columns || []).filter((column) => column.role === role);
}

function getPrimaryCategoryColumn(mapping: MaybeMapping): DatasusMappingColumn | null {
  return (mapping?.columns || []).find((column) => column.role === 'primary-category')
    || (mapping?.columns || []).find((column) => column.role === 'category')
    || null;
}

function collectCategoryColumns(mapping: MaybeMapping): DatasusMappingColumn[] {
  return (mapping?.columns || []).filter((column) => column.role === 'primary-category' || column.role === 'category');
}

function collectTimeColumns(mapping: MaybeMapping): DatasusMappingColumn[] {
  return roleColumns(mapping, 'time');
}

function collectMeasureColumns(mapping: MaybeMapping): DatasusMappingColumn[] {
  return roleColumns(mapping, 'measure');
}

function collectTotalColumns(mapping: MaybeMapping): DatasusMappingColumn[] {
  return roleColumns(mapping, 'total');
}

function metricOptionList(parsed: MaybeParsed, mapping: MaybeMapping): DatasusMetricOption[] {
  const usedKeys = new Set<string>();
  const formatType: DatasusFormatType = mapping?.formatType || parsed?.diagnosis?.formatType || 'hybrid';
...
function coerceTimeKey(rawTime: unknown): TimeKeyInfo {
...
function sortTimeValues(values: unknown[]): string[] {
...
function categoryLabelForRow(row: DatasusRow, categoryColumns: DatasusMappingColumn[], utils?: MaybeUtils): string {
...
function extraDimensionsForRow(row: DatasusRow, mapping: MaybeMapping, primaryCategoryColumn: DatasusMappingColumn | null): Record<string, string> {
...
function aggregateValuesForRow(row: DatasusRow, totalColumns: DatasusMappingColumn[], stats?: MaybeStats): Record<string, number | null> {
  return totalColumns.reduce<Record<string, number | null>>((accumulator, column) => {
    accumulator[column.header || `Coluna ${column.index + 1}`] = parseDatasusNumber(row.cleanCells[column.index], stats);
    return accumulator;
  }, {});
}

function buildWideRecords(
  source: DatasusSource,
  parsed: DatasusParsedOk,
  mapping: DatasusMapping,
  utils: MaybeUtils,
  stats: MaybeStats,
): { records: NormalizedDatasusRecord[]; metricOptions: DatasusMetricOption[] } {
  ...
  })).filter((item): item is { column: DatasusMappingColumn; option: DatasusMetricOption } => Boolean(item.option));

  const records: NormalizedDatasusRecord[] = [];
...
function buildLongRecords(
  ...
  const totalColumns = collectTotalColumns(mapping);
  const metricOptions = metricOptionList(parsed, mapping);
  const metricColumns: Array<{ column: DatasusMappingColumn | null; option: DatasusMetricOption }> = measureColumns.length
  ...
  const records: NormalizedDatasusRecord[] = parsed.bodyRows.map((row, rowOffset) => {
    const category = categoryLabelForRow(row, categoryColumns, utils);
    const categoryKey = normalizeDatasusToken(category);
...
function buildSummary(records: NormalizedDatasusRecord[], schema: NormalizedDatasusSchema): NormalizedDatasusSummary {
...
export function normalizeDatasusSource(source: DatasusSource, utils?: MaybeUtils, stats?: MaybeStats): NormalizedDatasusResult {
  ...
  const previewRows: NormalizedDatasusPreviewRow[] = normalized.records.slice(0, 20).map((record) => ({
...
export function getMetricOptions(source: DatasusSource | null | undefined): DatasusMetricOption[] {
  return source?.normalized?.schema?.metricOptions || [];
}

export function getPrimaryMetricKey(source: DatasusSource | null | undefined): string {
  return source?.normalized?.schema?.primaryMetricKey || getMetricOptions(source)[0]?.key || 'value';
}

export function getMetricLabel(source: DatasusSource | null | undefined, metricKey: string): string {
  return getMetricOptions(source).find((option) => option.key === metricKey)?.label || metricKey || 'Valor';
}

export function getCategoryOptions(source: DatasusSource | null | undefined, includeTotal = false): DatasusCategoryOption[] {
  const seen = new Map<string, DatasusCategoryOption>();
...
export function getTimeOptions(source: DatasusSource | null | undefined): DatasusTimeOption[] {
...
function isRecordSelected(
  record: NormalizedDatasusRecord,
  { categoryKeys, timeKeys, includeTotal }: { categoryKeys?: string[]; timeKeys?: string[]; includeTotal: boolean },
): boolean {
...
export function filterSourceRecords(source: DatasusSource | null | undefined, {
...
function summarizePerCategory(source: DatasusSource | null | undefined, {
  metricKey = getPrimaryMetricKey(source),
  categoryKeys = [],
  timeKeys = [],
  includeTotal = false,
  ...
  const buckets = new Map<string, { rowKey: string; rowLabel: string; values: number[]; validTimes: Set<string>; rawCount: number }>();
  filterSourceRecords(source, { metricKey, categoryKeys, timeKeys, includeTotal }).forEach((record) => {
    ...
    const bucket = buckets.get(key) as { rowKey: string; rowLabel: string; values: number[]; validTimes: Set<string>; rawCount: number };
...
function periodLabelFromKeys(source: DatasusSource | null | undefined, timeKeys: string[] | undefined): string {
...
export function findBestNormalizedPair(sources: DatasusSource[] | null | undefined): DatasusBestPairResult | null {
  if (!Array.isArray(sources) || sources.length < 2) return null;

  let best: DatasusBestPairResult | null = null;
...
export function deriveIndependentTTest({
  ...
  source: DatasusSource | null | undefined;
  metricKey?: string;
  groupAKeys?: string[];
  groupBKeys?: string[];
  timeKeys?: string[];
...
export function derivePairedTTest({
  ...
  const derivedRows: DatasusPairedRow[] = [];
  const omittedRows: DatasusOmittedRow[] = [];
...
function sameSourceCorrelation({
  ...
  const pairs = filterSourceRecords(source, {
    ...
    x: record.metricValues[xMetricKey] as number,
    y: record.metricValues[yMetricKey] as number,
    category: record.category,
    time: record.time,
  }));
...
function summarizeForAlignment(
  source: DatasusSource | null | undefined,
  metricKey: string,
  {
    categoryKeys = [],
...
): Map<string, { key: string; label: string; values: number[]; category: string; time: string; value: number }> {
  const buckets = new Map<string, { key: string; label: string; values: number[]; category: string; time: string }>();
  filterSourceRecords(source, { metricKey, categoryKeys, timeKeys, includeTotal }).forEach((record) => {
    ...
    (buckets.get(key) as { values: number[] }).values.push(record.metricValue as number);
  });
...
export function deriveCorrelationPairs({
  ...
  xSource: DatasusSource | null | undefined;
  ySource: DatasusSource | null | undefined;
  xMetricKey?: string;
  yMetricKey?: string;
  categoryKeys?: string[];
  timeKeys?: string[];
  ...
  const pairs = sameSource
    ...
    : (() => {
      ...
      return [...leftMap.entries()].reduce<DatasusCorrelationPair[]>((accumulator, [key, leftRow]) => {
...
export function derivePraisSeries({
  ...
  const buckets = new Map<string, { timeLabel: string; timeNumeric: number | null; values: number[] }>();
  ...
  const ordered: DatasusPraisSeriesRow[] = [...buckets.values()]
    .map((bucket, index) => ({
...
export function suggestTestsForSources(sources: DatasusSource[] | null | undefined): string[] {
  ...
  sources.forEach((source) => {
    ...
    if ((source.mapping?.columns.filter((c) => c.role === 'measure').length ?? 0) >= 2) {
