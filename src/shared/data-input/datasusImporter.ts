/**
 * Typed port of `assets/js/datasus-importer.js` (534 lines, ported in
 * full). Logic is byte-for-byte unchanged — header-candidate scoring
 * (`scoreHeaderCandidate`) and delimiter scoring (`scoreDelimiter`) are
 * tuned against real messy DataSUS/TABNET dumps and must not be "improved"
 * during this port (01-RESEARCH.md Pitfall 3).
 *
 * `splitDelimitedLine` here is a deliberate duplicate of the one in
 * `parseTabular.ts` — the two were tuned against different input shapes
 * and are intentionally NOT deduplicated into a shared helper (see
 * 01-03-SUMMARY.md).
 */

import type {
  DatasusColumnProfile,
  DatasusDiagnosis,
  DatasusFormatType,
  DatasusHeaderCandidate,
  DatasusLine,
  DatasusMapping,
...
function callOrFallback<T extends (...args: never[]) => unknown>(fn: T | undefined, fallback: T): T {
  return typeof fn === 'function' ? fn : fallback;
}

function baseNormalizeText(text: unknown): string {
  return String(text || '')
    .replace(/\u0000/g, '')
    .replace(/\r\n?/g, '\n')
    .normalize('NFC');
...
function baseNormalizeLabel(value: unknown): string {
  return baseNormalizeText(value).replace(/\s+/g, ' ').trim();
}

export function normalizeDatasusText(text: unknown, utils?: MaybeUtils): string {
  const normalizeText = callOrFallback(utils?.normalizeImportedText, baseNormalizeText);
  return normalizeText(String(text || ''));
}

export function normalizeDatasusLabel(value: unknown, utils?: MaybeUtils): string {
  const normalizeLabel = callOrFallback(utils?.normalizeImportedLabel, baseNormalizeLabel);
  return normalizeLabel(String(value || ''));
}

export function normalizeDatasusToken(value: unknown, utils?: MaybeUtils): string {
...
export function isMissingToken(value: unknown, utils?: MaybeUtils): boolean {
  return DEFAULT_MISSING_TOKENS.has(normalizeDatasusToken(value, utils));
}

export function parseDatasusNumber(raw: unknown, stats?: MaybeStats): number | null {
...
function dominantCount(values: string[]): { value: string | number; frequency: number } {
  const counts = new Map<string, number>();
  values.forEach((value) => {
    counts.set(value, (counts.get(value) || 0) + 1);
  });

  let winner: { value: string | number; frequency: number } = { value: 0, frequency: 0 };
...
function scoreDelimiter(lines: LineRecord[], delimiter: string, utils?: MaybeUtils): number {
  ...
  const winner = dominantCount(meaningful.map(String));
  const winnerValue = Number(winner.value);
  const average = meaningful.reduce((sum, value) => sum + value, 0) / meaningful.length;
  const spreadPenalty = meaningful.reduce((sum, value) => sum + Math.abs(value - winnerValue), 0);
  const bias = delimiter === ';' ? 0.35 : delimiter === '\t' ? 0.25 : 0.05;

  return (winner.frequency * 4.5) + (winnerValue * 2.2) + average + bias - spreadPenalty;
}

export function detectDatasusDelimiter(lines: LineRecord[], utils?: MaybeUtils): string {
  const candidates = [';', '\t', ','];
...
export function isYearLikeToken(value: unknown, utils?: MaybeUtils): boolean {
  return /^(18|19|20)\d{2}$/.test(normalizeDatasusToken(value, utils));
}

export function isTimeLikeToken(value: unknown, utils?: MaybeUtils): boolean {
...
export function isTotalLikeToken(value: unknown, utils?: MaybeUtils): boolean {
  const token = normalizeDatasusToken(value, utils);
  return token === 'total' || token.startsWith('total ');
}

export function cleanDatasusCategoryLabel(value: unknown, utils?: MaybeUtils): string {
...
function buildLineObjects(text: unknown, utils?: MaybeUtils): DatasusLine[] {
...
function buildRowMatrix(lines: DatasusLine[], delimiter: string, utils?: MaybeUtils): DatasusRow[] {
...
function headerKeywordHits(cells: string[], utils?: MaybeUtils): number {
...
export function buildHeaderCandidates(rowMatrix: DatasusRow[], utils?: MaybeUtils, stats?: MaybeStats): DatasusHeaderCandidate[] {
  const upperLimit = Math.min(rowMatrix.length, 24);
  const candidates: DatasusHeaderCandidate[] = [];
...
function padCells(cells: string[], size: number): string[] {
  const padded = [...cells];
...
function profileColumn(
  { header, values, index }: { header: string; values: string[]; index: number },
  utils?: MaybeUtils,
  stats?: MaybeStats,
): DatasusColumnProfile {
  ...
  let suggestedRole: Exclude<DatasusColumnProfile['suggestedRole'], never> = 'category';
  ...
  const suggestedType: DatasusVariableType = suggestedRole === 'time'
    ? 'temporal'
    : suggestedRole === 'measure'
      ? 'quantitative'
      : suggestedRole === 'total'
...
function detectProbableFormat(columnProfiles: DatasusColumnProfile[]): DatasusFormatType {
...
function findPrimaryCategoryColumn(columnProfiles: DatasusColumnProfile[], formatType: DatasusFormatType): DatasusColumnProfile | null {
...
function buildDiagnosis({
  ...
  primaryCategory: DatasusColumnProfile | null;
  bodyRows: DatasusRow[];
  utils?: MaybeUtils;
}): DatasusDiagnosis {
  const timeColumns = columnProfiles.filter((profile) => profile.suggestedRole === 'time');
...
export function createInitialDatasusMapping(parsed: { columnProfiles: DatasusColumnProfile[]; diagnosis: DatasusDiagnosis; headerRowIndex: number }): DatasusMapping {
  const columns: DatasusMappingColumn[] = parsed.columnProfiles.map((profile) => {
    let role: DatasusColumnProfile['suggestedRole'] | 'primary-category' = profile.suggestedRole;
    if (parsed.diagnosis.primaryCategoryIndex === profile.index) {
      role = 'primary-category';
    }

    const variableType: DatasusVariableType = role === 'primary-category' || role === 'category'
      ? 'categorical'
      : role === 'time'
        ? 'temporal'
        : role === 'measure'
...
export function parseDatasusText({
  text,
  fileName = '',
  utils,
  stats,
  ...
  const effectiveHeaderRowIndex = Number.isInteger(headerRowIndex)
    ? Math.max(0, Math.min(headerRowIndex as number, rowMatrix[rowMatrix.length - 1].lineIndex))
  ...
  const normalizedBodyRows: DatasusRow[] = bodyRows.map((row) => ({
    ...row,
  ...
  return {
    ...parsed,
    initialMapping: createInitialDatasusMapping(parsed),
  };
}
