...
describe('kruskalEngine validation', () => {
  it.each(['constructor', 'toString', '__proto__'])('keeps the arbitrary group label %s through rank summaries', (reservedLabel) => {
    ...
    expect(runAnalysis(dataset).groupSummaries[reservedLabel]).toMatchObject({ n: 2, median: 1.5 });
  });
